...
---- CHANGELOG -----
-- ondrase, presets now work for both autogroups and regular groups
...
local function loadAutogroupPresetHandler(cmd, optLine, optWords, data, isRepeat, release, actions)
	...
	if not presets[newPreset] then
		Echo("warn no preset in:loadAutogroupPresetHandler")
		return
	end
	local prevGroup = presets[currPreset]
	local prevpreset = currPreset
	for j = 0, 9 do
		regularpresets[prevpreset][j]={}
	end
	...
	for _, uID in ipairs(Spring.GetTeamUnits(myTeam)) do
		...
		if  GetUnitGroup(uID) ~= nil then -- if in last
			table.insert(regularpresets[prevpreset][GetUnitGroup(uID)],uID)
			SetUnitGroup(uID, -1)
		end
	...
	for i = 0, 9 do
		...
		for j = 1, #regularpresets[newPreset][i] do
			--Echo({regularpresets[newPreset][i][j]})
			SetUnitGroup(regularpresets[newPreset][i][j],i)
		end
	end
end
